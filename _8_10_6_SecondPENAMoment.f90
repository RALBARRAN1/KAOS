module SecondPENAMoment

! ----------------------------------------------------
! ----------------------------------------------------

! %%%%%%	8.10.6 SECOND P-COMPONENT ENA MOMENT:	%%%%%%
...
! COMPUTE SECOND ENA MOMENT (P-ENERGY) OF PHASE-SPACE DISTRIBUTION FUNCTIONS:

	subroutine SecondPENAMomentSub
		...
		do nn= 1, SpecieT(s)%FluxTubeT(f)%NNtT(1)+ 1, 1
			if (((n == 1) .and. (nn == 1)) .or. ((n /= 1) .and. (nn /= 1) .and. &
				...
				do Qind= SpecieT(s)%FluxTubeT(f)%NqLBT(1), SpecieT(s)%FluxTubeT(f)%NqUBT(1), 1
					...
					! COMPUTE ENA P-ENERGY (SECOND MOMENT):
					...
					if (rank == 0) then
								...
								do Vphiind= 1, SpecieT(s)%FluxTubeT(f)%QCellT(1)%NVphiGT(1), 1
									...
									if (SpecieT(s)%FluxTubeT(f)%QCellT(Qind)% &
										...
										gggENA(Vpind, Vqind, Vphiind, nn)= &
											...
											SpecieT(s)%FluxTubeT(f)%QCellT(1)% &
											V3CellT(Vpind, Vqind, Vphiind)%VpGCGT(1))**2d0))* &
									...
									if ((isnan(real(gggENA(Vpind, Vqind, Vphiind, nn))) &
										...
										write(*, *) achar(27) // '[33m ERROR: RANK= ', rank, &
											...
											' SECOND P-COMPONENT ENA MOMENT SUBROUTINE' &
											// achar(27) // '[0m.'
									end if
									...
									if (((SpecieT(s)%FluxTubeT(f)%QCellT(Qind)% &
										...
										((abs((SpecieT(s)%FluxTubeT(f)%M1PphENART(nn, Qind)- &
										SpecieT(s)%FluxTubeT(f)%QCellT(1)% &
										V3CellT(Vpind, Vqind, Vphiind)%VpGCGT(1))**2d0)) /= 0) .and. &
						...
						if (SpecieT(s)%FluxTubeT(f)%M0phENART(nn, Qind) == 0) then
							SpecieT(s)%FluxTubeT(f)%M2PphENART(nn, Qind)= 0d0
						else if (SpecieT(s)%FluxTubeT(f)%M0phENART(nn, Qind) /= 0) then
							SpecieT(s)%FluxTubeT(f)%M2PphENART(nn, Qind)= &
						...
						if ((SpecieT(s)%FluxTubeT(f)%QCellT(Qind)%NqENART(nn) == 0) .and. &
							(SpecieT(s)%FluxTubeT(f)%M2PphENART(nn, Qind) /= 0)) then
							write(*, *) achar(27) // '[33m ERROR: RANK= ', rank, &
								' INCONSISTENT M2PphENART and NqENART VALUE FOR SPECIE= ', s, &
								', FLUX TUBE= ', f, ', Qind= ', Qind, ', AND MASTER', &
								' TIME-STEP= ', nn, ' IN SECOND P-COMPONENT ENA MOMENT', &
						...
						if ((sum(SpecieT(s)%FluxTubeT(f)%QCellT(Qind)%FphENARTp(:, :, :, nn)) == 0) .and. &
							(SpecieT(s)%FluxTubeT(f)%M2PphENART(nn, Qind) /= 0)) then
							write(*, *) achar(27) // '[33m ERROR: RANK= ', rank, &
								' INCONSISTENT M2PphENART and FphENARTp SUMMATION FOR SPECIE= ', s, &
								', FLUX TUBE= ', f, ', Qind= ', Qind, ', AND MASTER', &
								' TIME-STEP= ', nn, ' IN SECOND P-COMPONENT ENA MOMENT', &
								' SUBROUTINE' // achar(27) // '[0m.'
						end if
						...
						if (((sum(gggENA(:, :, :, nn)) /= 0) .and. &
							(SpecieT(s)%FluxTubeT(f)%M2PphENART(nn, Qind) == 0)) .or. &
							((sum(gggENA(:, :, :, nn)) == 0) .and. &
							(SpecieT(s)%FluxTubeT(f)%M2PphENART(nn, Qind) /= 0))) then
							write(*, *) achar(27) // '[33m ERROR: RANK= ', rank, &
								' INCONSISTENT M2PphENART and INTEGRAND SUMMATION FOR SPECIE= ', s, &
								', FLUX TUBE= ', f, ', Qind= ', Qind, ', AND MASTER', &
								' TIME-STEP= ', nn, ' IN SECOND P-COMPONENT ENA MOMENT', &
								' SUBROUTINE' // achar(27) // '[0m.'
						end if
...
! 								SpecieT(s)%FluxTubeT(f)%M2PphENART(nn, Qind)
! 							write(*, *) 'M2PENA REFR s, f, nn, Qind= ', s, f, nn, Qind, &
! 								SpecieT(s)%FluxTubeT(f)%EnergyPENAREFRT(nn, Qind)
! 						end if
! 					end if
! 				end do
! 			end if
! 		end do
	...
	end subroutine SecondPENAMomentSub

end module SecondPENAMoment
