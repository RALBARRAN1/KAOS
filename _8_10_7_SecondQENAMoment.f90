module SecondQENAMoment

! ----------------------------------------------------
! ----------------------------------------------------

! %%%%%%	8.10.7 SECOND Q-COMPONENT ENA MOMENT:	%%%%%%
...
! COMPUTE SECOND ENA MOMENT (Q-ENERGY) OF PHASE-SPACE DISTRIBUTION FUNCTIONS:

	subroutine SecondQENAMomentSub
		...
		do nn= 1, SpecieT(s)%FluxTubeT(f)%NNtT(1)+ 1, 1
			if (((n == 1) .and. (nn == 1)) .or. ((n /= 1) .and. (nn /= 1) .and. &
				...
				do Qind= SpecieT(s)%FluxTubeT(f)%NqLBT(1), SpecieT(s)%FluxTubeT(f)%NqUBT(1), 1
					...
					! COMPUTE ENA Q-ENERGY (SECOND MOMENT):
					...
					if (rank == 0) then
								...
								do Vphiind= 1, SpecieT(s)%FluxTubeT(f)%QCellT(1)%NVphiGT(1), 1
									...
									if (SpecieT(s)%FluxTubeT(f)%QCellT(Qind)% &
										...
										gggENA(Vpind, Vqind, Vphiind, nn)= &
											...
											SpecieT(s)%FluxTubeT(f)%QCellT(1)% &
											V3CellT(Vpind, Vqind, Vphiind)%VqGCGT(1))**2d0))* &
									...
									if (((SpecieT(s)%FluxTubeT(f)%QCellT(Qind)% &
										...
										((abs((SpecieT(s)%FluxTubeT(f)%M1QphENART(nn, Qind)- &
										SpecieT(s)%FluxTubeT(f)%QCellT(1)% &
										V3CellT(Vpind, Vqind, Vphiind)%VqGCGT(1))**2d0)) /= 0) .and. &
						...
						if (SpecieT(s)%FluxTubeT(f)%M0phENART(nn, Qind) == 0) then
							SpecieT(s)%FluxTubeT(f)%M2QphENART(nn, Qind)= 0d0
						else if (SpecieT(s)%FluxTubeT(f)%M0phENART(nn, Qind) /= 0) then
							SpecieT(s)%FluxTubeT(f)%M2QphENART(nn, Qind)= &
						...
						if (((sum(gggENA(:, :, :, nn)) /= 0) .and. &
							(SpecieT(s)%FluxTubeT(f)%M2QphENART(nn, Qind) == 0)) .or. &
							((sum(gggENA(:, :, :, nn)) == 0) .and. &
							(SpecieT(s)%FluxTubeT(f)%M2QphENART(nn, Qind) /= 0))) then
							write(*, *) achar(27) // '[33m ERROR: RANK= ', rank, &
...
! 							write(*, *) 'M2QphENART s, f, nn, Qind= ', s, f, nn, Qind, &
! 								SpecieT(s)%FluxTubeT(f)%M2QphENART(nn, Qind)
! 							write(*, *) 'M2QENA REFR s, f, nn, Qind= ', s, f, nn, Qind, &
! 								SpecieT(s)%FluxTubeT(f)%EnergyQENAREFRT(nn, Qind)
! 						end if
! 					end if
! 				end do
! 			end if
! 		end do
	...
	end subroutine SecondQENAMomentSub

end module SecondQENAMoment
