module FirstPHIENAMoment

! ----------------------------------------------------
! ----------------------------------------------------

! %%%%%%	8.10.4 FIRST PHI-COMPONENT ENA MOMENT:	%%%%%%
...
! COMPUTE FIRST PHI-COMPONENT ENA MOMENT (PHI-VELOCITY) OF PHASE-SPACE DISTRIBUTION FUNCTIONS:

	subroutine FirstPHIENAMomentSub
		...
		do nn= 1, SpecieT(s)%FluxTubeT(f)%NNtT(1)+ 1, 1
			if (((n == 1) .and. (nn == 1)) .or. ((n /= 1) .and. (nn /= 1) .and. &
				...
				do Qind= SpecieT(s)%FluxTubeT(f)%NqLBT(1), SpecieT(s)%FluxTubeT(f)%NqUBT(1), 1
					...
					! COMPUTE ENA PHI-COMPONENT VELOCITY (FIRST PHI MOMENT):
					...
					if (rank == 0) then
								...
								do Vphiind= 1, SpecieT(s)%FluxTubeT(f)%QCellT(1)%NVphiGT(1), 1
									...
									if (SpecieT(s)%FluxTubeT(f)%QCellT(Qind)% &
										...
										gggENA(Vpind, Vqind, Vphiind, nn)= &
											...
											(SpecieT(s)%FluxTubeT(f)%QCellT(1)% &
											V3CellT(Vpind, Vqind, Vphiind)%VphiGCGT(1))* &
									...
									if ((isnan(real(gggENA(Vpind, Vqind, Vphiind, nn))) &
										...
										write(*, *) achar(27) // '[33m ERROR: RANK= ', rank, &
											...
											', AND STATISTICAL TIME-STEP= ', nn, ' IN FIRST', &
											' PHI-COMPONENT ENA MOMENT SUBROUTINE' &
											// achar(27) // '[0m.'
									end if
									...
									if (((SpecieT(s)%FluxTubeT(f)%QCellT(Qind)% &
										...
										(SpecieT(s)%FluxTubeT(f)%QCellT(1)% &
										V3CellT(Vpind, Vqind, Vphiind)%VphiGCGT(1) /= 0) .and. &
						...
						if (SpecieT(s)%FluxTubeT(f)%M0phENART(nn, Qind) == 0) then
							SpecieT(s)%FluxTubeT(f)%M1PHIphENART(nn, Qind)= 0d0
						else if (SpecieT(s)%FluxTubeT(f)%M0phENART(nn, Qind) /= 0) then
							SpecieT(s)%FluxTubeT(f)%M1PHIphENART(nn, Qind)= &
						...
						if ((SpecieT(s)%FluxTubeT(f)%QCellT(Qind)%NqENART(nn) == 0) .and. &
							(SpecieT(s)%FluxTubeT(f)%M1PHIphENART(nn, Qind) /= 0)) then
							write(*, *) achar(27) // '[33m ERROR: RANK= ', rank, &
								' INCONSISTENT M1PHIphENART and NqENART VALUE FOR SPECIE= ', s, &
								', FLUX TUBE= ', f, ', Qind= ', Qind, ', AND STATISTICAL', &
								' TIME-STEP= ', nn, ' IN FIRST PHI-COMPONENT ENA MOMENT', &
						...
						if ((sum(SpecieT(s)%FluxTubeT(f)%QCellT(Qind)%FphENARTp(:, :, :, nn)) == 0) .and. &
							(SpecieT(s)%FluxTubeT(f)%M1PHIphENART(nn, Qind) /= 0)) then
							write(*, *) achar(27) // '[33m ERROR: RANK= ', rank, &
								' INCONSISTENT M1PHIphENART and FphENARTp SUMMATION FOR SPECIE= ', s, &
								', FLUX TUBE= ', f, ', Qind= ', Qind, ', AND STATISTICAL', &
								' TIME-STEP= ', nn, ' IN FIRST PHI-COMPONENT ENA MOMENT', &
								' SUBROUTINE' // achar(27) // '[0m.'
						end if
						...
						if (((sum(gggENA(:, :, :, nn)) /= 0) .and. &
							(SpecieT(s)%FluxTubeT(f)%M1PHIphENART(nn, Qind) == 0)) .or. &
							((sum(gggENA(:, :, :, nn)) == 0) .and. &
							(SpecieT(s)%FluxTubeT(f)%M1PHIphENART(nn, Qind) /= 0))) then
							write(*, *) achar(27) // '[33m ERROR: RANK= ', rank, &
								' INCONSISTENT M1PHIphENART and INTEGRAND SUMMATION FOR SPECIE= ', s, &
								', FLUX TUBE= ', f, ', Qind= ', Qind, ', AND STATISTICAL', &
								' TIME-STEP= ', nn, ' IN FIRST PHI-COMPONENT ENA MOMENT', &
								' SUBROUTINE' // achar(27) // '[0m.'
						end if
...
! 					if (rank == 0) then
! 						if (SpecieT(s)%FluxTubeT(f)%M0phENART(nn, Qind) /= 0) then
! 							write(*, *) 'M1PHIphENART s, f, nn, Qind= ', s, f, nn, Qind, &
! 								SpecieT(s)%FluxTubeT(f)%M1PHIphENART(nn, Qind)
! 							write(*, *) 'M1PHIENA REFR s, f, nn, Qind= ', s, f, nn, Qind, &
...
! 		end do
	...
	end subroutine FirstPHIENAMomentSub

end module FirstPHIENAMoment
